...
namespace ORCA{
	const size_t ORCA_MAX_LEAF_SIZE = 10;
	...
	void KdTree::buildAgentTreeRecursive(size_t begin, size_t end, size_t node){
		...
		if(end - begin > ORCA_MAX_LEAF_SIZE){
			/* No leaf node. */
			size_t coord;
	...
	{
		if (agentTree_[node].end - agentTree_[node].begin <= ORCA_MAX_LEAF_SIZE) {
	...
	void KdTree::setAgents(std::vector<Agent *> x){
		agents_ = x;
	}
}