/*
 * @file Agent.cpp
 * @brief Agent implementations, for multiple UAVs collision avoidance
 *
...
namespace ORCA{
	...
	void Agent::computeNeighbors()
	{
		...
		if (maxNeighbors_ > 0) {
			tree_->computeAgentNeighbors(this, neighborDist_ * neighborDist_);
		}
	}
	...
	void Agent::computeNewVelocity(){
		...
		for (size_t i = 0; i < agentNeighbors_.size(); ++i) {
			...
			const float combinedRadius = 2*radius_;
			...
			else { //TOD0 CONTROL  invTimeStep
				/* Collision. */
				const float invTimeStep = 1.0f / 1.0f;
	...
	bool linearProgram1(const std::vector<Plane> &planes, size_t planeNo, const Line &line, float radius, const Vector3 &optVelocity, bool directionOpt, Vector3 &result)
	{
		...
		for (size_t i = 0; i < planeNo; ++i) {
			...
			if (sqr(denominator) <= ORCA_EPSILON) {
	...
	bool linearProgram2(const std::vector<Plane> &planes, size_t planeNo, float radius, const Vector3 &optVelocity, bool directionOpt, Vector3 &result)
	{
		...
		if (directionOpt) {
			...
			if (planeOptVelocityLengthSq <= ORCA_EPSILON) {
				result = planeCenter;
			}
			else {
		...
		for (size_t i = 0; i < planeNo; ++i) {
			if (planes[i].normal * (planes[i].point - result) > 0.0f) {
				...
				if (absSq(crossProduct) <= ORCA_EPSILON) {
	...
	// void Agent::position_cb(ignition::math::Vector3d current_position){ 

    //     position_[0] = (double)current_position.X();
    //     position_[1] = (double)current_position.Y();
    //     position_[2] = (double)current_position.Z();
    // }

 	// void Agent::vel_cb(ignition::math::Vector3d current_velocity){

    // 	velocity_[0] = (double)current_velocity.X();
    // 	velocity_[1] = (double)current_velocity.Y();
    // 	velocity_[2] = (double)current_velocity.Z();
	// }
